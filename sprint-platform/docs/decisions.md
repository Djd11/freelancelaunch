...
| D9 | Content | **LLM-only content generation.** Deterministic lesson/project/mentor templates removed; a failed/unusable LLM output surfaces a visible error (`generation_error` on the day payload, 503 on the mentor turn) instead of template content. Provider chain (env → OpenRouter → Omniroute) stays as availability redundancy. | The learner is paying for job-grounded content — a template pretending to be generated content is the worst failure mode (supersedes the v1 "graceful degradation" carry-over for LLM content). |
