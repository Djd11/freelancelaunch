...
- **Watch · Lesson:** TwoPanel JS player — a pre-built Remotion composition (`static/video/lesson-player.js`) with kinetic text + edge-tts voiceover, played in-browser (no MP4; YouTube distribution is a later concern). The lesson script + key points are generated per day from the cluster's live job posting (`lesson_engine.lesson_for_day`, **LLM-only** — no deterministic content) and stored in `sprint_days.action_payload.lesson`; the async content worker also generates the voiceover (`video_engine` → edge-tts → `voiceovers` Storage bucket) and stores `lesson.voiceover = {url, duration_seconds}`. When the LLM fails the worker stamps a visible `generation_error` on the day payload (never template content); when no voiceover exists yet, the day view renders the kinetic-text fallback.
...
- **Copy-work projects are job-grounded, not a hard-coded email template:** the 3 replication projects are seeded as a placeholder skeleton (`copywork_engine.create_projects`) and the async worker fills `clone_steps`/`rubric` from the cluster's live job posting (`lesson_engine.project_anatomy`, **LLM-only**). A learner in ANY cluster gets copy-work tasks for their own job — never a fixed "Abandoned-Cart" flow (fix H1).
- **Day mapping is complete:** all four Phase A copy-work days map to a project — Days 2–3 → project 1, Day 4 → project 2, Day 5 → project 3 — so project 3 is reachable and Gate A can pass through the real day flow (fix C1).
- **Gap-Fill preview card:** auto-detected nuance from the previous project ("mobile responsiveness", carried on copy-work project 2's `gap_fill_topic`) → "Day 5 serves a targeted 30-min micro-lesson." **Day 5's lesson IS that micro-lesson** — the worker generates a targeted Gap-Fill lesson on the flagged nuance, not another generic copy-work lesson (fix H2).
...
- Backend: request-scoped RAG over (job description + lesson content + progress), LLM provider chain, 20s timeout; LLM-only (failure → visible 503 error, no template content).
...
- **Cluster content library — cohort amortization (implemented, arch §2 P3):** the default enrollment path consumes the **`cluster_content_library`** table — the 14 lessons + 3 project anatomies are provisioned **ONCE per cluster** by the admin under `/admin/content/<cluster>` (LLM generation under admin control, JSON edit per day, or seed-from-sprint) and applied to every new sprint with **zero per-user LLM calls**. The per-sprint LLM worker is now only the **fallback** for clusters whose library is not ready — this is the fix for individual users failing with "generation failed" when free-tier LLM rate limits could not absorb ~31 calls per enrollment. Library rows carry `origin` (`generated`/`edited`); regeneration never clobbers edited content. BDD: `docs/bdd/content-library.feature`.
