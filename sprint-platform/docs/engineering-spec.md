...
- **Copy-work projects are job-grounded, not a hard-coded email template:** the 3 replication projects are seeded as a skeleton (`copywork_engine.create_projects`) and the async worker fills `clone_steps`/`rubric` from the cluster's live job posting (`lesson_engine.project_anatomy`, LLM → deterministic job-grounded fallback). A learner in ANY cluster gets copy-work tasks for their own job — never a fixed "Abandoned-Cart" flow (fix H1).
...
- **Gap-Fill detection** (Phase A) is deterministic in v1 — the missing nuance lives on copy-work project 2's `gap_fill_topic`, surfaces on the day view before Day 5, and **Day 5's generated lesson is the targeted 30-min micro-lesson on that nuance** (`lesson_engine._gap_fill_lesson`).
